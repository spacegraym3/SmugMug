...
Ext.ux.Cover = Ext.extend(Ext.DataView, {
	...
	refresh: function(){
		...
		this.all.each(function(item, el, idx) {
			...
			*/
			item.setBox(itemBox);
			// chwhite item.down('.'+this.itemBaseCls).setBox({height: itemBox.height/1.5, width: itemBox.width});
			item.down('.'+this.itemBaseCls).setBox({height: itemBox.height/1.1, width: itemBox.width});
		}, this);
